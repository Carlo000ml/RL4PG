...
def main(settings):
    ...
    runs_folder = (
        config["hyperparameters"]["save folder"]
        + f'Experiment_{datetime.now().strftime("%Y-%m-%d_%H-%M")}'
        + "_"
        + config["hyperparameters"].get("run name", "")
    )
    ...
    agents_kargs_text = "\n".join(
        [f"{key}: {value}" for key, value in agents_kargs.items()]
    )
    policy_kargs_text = "\n".join(
        [f"{key}: {value}" for key, value in policy_kargs.items()]
    )
    Q_value_estimator_kargs_text = "\n".join(
        [f"{key}: {value}" for key, value in Q_value_estimator_kargs.items()]
    )
    MA_manager_kargs_text = "\n".join(
        [f"{key}: {value}" for key, value in MA_manager_kargs.items()]
    )
    gp_kargs_text = "\n".join([f"{key}: {value}" for key, value in gp_kargs.items()])
    replay_buffer_kargs_text = "\n".join(
        [f"{key}: {value}" for key, value in replay_buffer_kargs.items()]
    )
    ...
    GlobalEnv = GlobalEnvironment(
        env_train, build_graph=build_torch_sub_graph, device=device
    )
    GlobalEnv_val = GlobalEnvironment(
        env_val, build_graph=build_torch_sub_graph, device=device
    )
    GlobalEnv_test = GlobalEnvironment(
        env_test, build_graph=build_torch_sub_graph, device=device
    )
    ...
    Sub_Agents = {
        i: DQN_Agent(
            action_space=build_sub_action_space(
                env_train, i, action_type="set", N_1=agents_kargs["N_1"]
            ),  # N_1_secure_action_space(env_train,i),
            agent_id=i,
    ...
    if hyperparameters["learn demonstrations"]:
        ...
        if experiences_path_file.exists() and MA_experiences_path_file.exists():
            ...
            episode_studied = EpisodeData.list_episode(
                config["environment"]["Expert interaction path"]
            )  # To be configured later for better handling of the path

            a_c = {
                i: Action_Converters_sub(env_train, i, agents_kargs["N_1"])
            ...
            MA_exp = []
            print(
                f">>> Collecting demonstrations of {agents_kargs['demonstrations n']} steps"
            )
            for i in tqdm(range(len(episode_studied))):
                this_episode = EpisodeData.from_disk(*episode_studied[i])
                experiences, MA_exp = collect_n_step_expert_experiences_RL_Manager_sub(
                    this_episode,
                    n=agents_kargs["demonstrations n"],
                    action_converters=a_c,
                    build_graph=build_torch_sub_graph,
                    playable_subs=MultiAgent_Controll.playable,
            ...
            pathlib.Path(experiences_path_file).parent.mkdir(
                parents=True, exist_ok=True
            )
            pathlib.Path(MA_experiences_path_file).parent.mkdir(
                parents=True, exist_ok=True
            )
        ...
        print(
            f">>> Learning manager demonstrations for {MultiAgent_Controll.num_training_iters * MA_manager_kargs['n demonstrations training']} iterations"
        )
        ...
        print(
            f">>> Learning agents demonstrations for {130*agents_kargs['n training']} iterations"
        )
        ...
        print(f"No demonstrations learning")
    ...
    for episode in tqdm(range(hyperparameters["number of episodes"])):
        if (
            episode % hyperparameters["exploit interval"]
            < hyperparameters["exploit tests"]
            and start_exploit
        ):
            exploit_res = exploit_episode(
                Agents=Sub_Agents,
                MA_Controller=MultiAgent_Controll,
                Env=GlobalEnv,
                options=options,
            )
            writer.add_scalar(
                "Cumulative L2RPN Reward - EXPLOIT",
                exploit_res["cum reward"],
                exploit_step,
            )
            writer.add_scalar(
                "survive time - EXPLOIT", exploit_res["survive time"], exploit_step
            )
            writer.add_scalar(
                "Do action - EXPLOIT", exploit_res["Do action"], exploit_step
            )
            exploit_step += 1

        else:
            #
            train_res = train_episode(
                Agents=Sub_Agents,
                MA_Controller=MultiAgent_Controll,
                Env=GlobalEnv,
                options=options,
            ...
            if np.any(
                np.unique(np.array(agents_counter), return_counts=True)[1]
                > agents_kargs["start_training_capacity"]
            ):
                start_exploit = True  # start exploit only when the training has started
            else:
                start_exploit = False
            writer.add_histogram(
                "Buffer sizes",
                np.array(agents_counter),
                global_step=episode,
                bins=env_train.n_sub,
            )
            writer.add_scalar(
                "Manager buffer size", len(MultiAgent_Controll.buffer), episode
            )

            writer.add_scalar(
                "Cumulative L2RPN Reward", train_res["cum reward"], episode
            )
        ...
        if hyperparameters["validation"]:
            if episode % hyperparameters["validation frequency"] == 0:
                ...
                for chr_id in val_chr_ids:
                    val_res = exploit_episode(
                        Agents=Sub_Agents,
                        MA_Controller=MultiAgent_Controll,
                        Env=GlobalEnv_val,
                        options={"time serie id": chr_id},
                    )
                    writer.add_scalar(
                        "Cumulative L2RPN Reward - VAL", val_res["cum reward"], val_step
                    )
                    writer.add_scalar(
                        "Survive time - VAL", val_res["survive time"], val_step
                    )
                ...
                if not do_nothing_validation:
                    for chr_id in val_chr_ids:
                        do_n_res = do_nothing_episode(
                            GlobalEnv_val, options={"time serie id": chr_id}
                        )
                        writer.add_scalar(
                            "Cumulative L2RPN Reward do nothing",
                            do_n_res["cum reward"],
                            do_nothing_step,
                        )
                        writer.add_scalar(
                            "Survive time do nothing",
                            do_n_res["survive time"],
                            do_nothing_step,
                        )
                        do_nothing_step += 1
        ...
        if hyperparameters["test"] and test:
            for chr_id in test_chr_ids:
                ...
                writer.add_scalar(
                    "Cumulative L2RPN Reward - TEST", test_res["cum reward"], test_step
                )
                writer.add_scalar(
                    "Survive time - TEST", test_res["survive time"], test_step
                )
            ...
            if not do_nothing_test:
                for chr_id in test_chr_ids:
                    do_n_res = do_nothing_episode(
                        GlobalEnv_test, options={"time serie id": chr_id}
                    )
                    writer.add_scalar(
                        "Cumulative L2RPN Reward do nothing - TEST",
                        do_n_res["cum reward"],
                        do_nothing_step_test,
                    )
                    writer.add_scalar(
                        "Survive time do nothing - TEST",
                        do_n_res["survive time"],
                        do_nothing_step_test,
                    )
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Run experiment using JSON configuration."
    )
    parser.add_argument(
