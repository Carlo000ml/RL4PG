...
class MultiAgent_RL_Line_Manager:
    ...
    def _training_iteration(self):
        if self.buffer_type == "basic":
            states, actions, rewards, next_states, dones, _, _ = self.buffer.sample(
                self.batch_size, way="s-a-r-ns-d", demonstrations=False
            )
    ...
    def _demonstration_training_iteration(self):
        ...
        loss, _ = self.compute_demonstration_loss(
            states=states,
            actions=actions,
            rewards=rewards,
        ...
        if self.log_loss:
            self.writer.add_scalar(
                "Demonstration Loss", loss.item(), self.demonstration_training_iter
            )
    ...
    def update_parameters(self, loss):
        ...
        torch.nn.utils.clip_grad_value_(
            self.main_net.parameters(), self.gradient_clipping
        )
        self.optimizer.step()
    ...
    def compute_loss(
        ...
        if IS_weights is None and self.buffer_type == "basic":
            ...
            with torch.no_grad():
                if self.use_double:
                    best_main_actions = (
                        self.compute_main_q_values(batch_next_state, train=False)
                        .argmax(dim=1, keepdim=True)
                        .detach()
                    )
                    target_q_values = (
                        self.compute_target_q_values(batch_next_state)
                        .gather(1, best_main_actions)
                        .view(-1)
                    )
                else:
                ...
                target_q_values = (
                    batch_reward + (1 - batch_done) * self.gamma * target_q_values
                )
            ...
            loss = (
                torch.tensor(IS_weights, device=self.device) * TD_errors.pow(2)
            ).sum()

            return loss, TD_errors.detach().cpu().numpy()

    def compute_demonstration_loss(
        self, states, actions, rewards, next_states, dones, states_tn, dones_tn
    ):
        ...
        (
            batch_size,
            n_actions,
        ) = q_values.shape  # number of rows , number of columns of the q_values
        q_values = torch.gather(q_values, 1, actions).view(-1)
        ...
        with torch.no_grad():
            if self.use_double:
                best_main_actions = (
                    self.compute_main_q_values(next_states, train=False)
                    .argmax(dim=1, keepdim=True)
                    .detach()
                )
                target_q_values = (
                    self.compute_target_q_values(next_states)
                    .gather(1, best_main_actions)
                    .view(-1)
                )
            else:
        ...
        with torch.no_grad():
            if self.use_double:
                best_main_actions = (
                    self.compute_main_q_values(states_tn, train=False)
                    .argmax(dim=1, keepdim=True)
                    .detach()
                )
                target_q_values = (
                    self.compute_target_q_values(states_tn)
                    .gather(1, best_main_actions)
                    .view(-1)
                )
            else:
            ...
            rewards = torch.sum(
                rewards * self.gamma**exponents, axis=1
            )  # shape (batch_size)

            target_q_values = rewards + self.gamma**n * target_q_values * (
                1 - dones_tn
            )
        ...
        supervised_loss = torch.clip(
            torch.max(self.compute_main_q_values(states, train=True) + margin)
            - q_values,
            0,
        ).mean()
        ...
        return (
            single_step_loss
            + self.lambda1 * n_step_loss
            + self.lambda2 * supervised_loss
            + self.lambda3 * l2_norm,
            None,
        )
    ...
    def learn(self):
        ...
        if self.policy_type == "epsilon-greedy":
            ...
            self.writer.add_scalar(
                "Temperature", self.policy.temperature, self.training_count
            )
...
def playable_substations(env, n_1=True):
    ...
    for i in range(env.n_sub):
        if n_1:
            ...
            acsp = env.action_space.get_all_unitary_topologies_set(
                env.action_space, sub_id=i, add_alone_line=False
            )
