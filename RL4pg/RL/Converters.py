import numpy as np
from ..utils import (
    build_line_action_space,
    build_line_extremities_sub_indexes,
    build_sub_action_space,
)
import torch
...
class Reward_Converter_Line:
    ...
    def __init__(self, line_id, obs):
        ...
        substation_is_extremity_for_lines = np.where(
            obs.line_ex_to_subid == self.sub_0
        )[0]
        self.lines_0 = np.hstack(
            [substation_is_origin_for_lines, substation_is_extremity_for_lines]
        )

        # sub 1
        substation_is_origin_for_lines = np.where(obs.line_or_to_subid == self.sub_1)[0]
        substation_is_extremity_for_lines = np.where(
            obs.line_ex_to_subid == self.sub_1
        )[0]
        self.lines = np.unique(
            np.hstack(
                [
                    substation_is_origin_for_lines,
                    substation_is_extremity_for_lines,
                    self.lines_0,
                ]
            )
        )
    ...
    def grid2op_to_gym(self, obs):
        ...
        return (
            np.sum((1 - rho) ** 2) / self.n_lines
        )  # ( (   np.sum(   (1-rho)**2   )   )    /self.n_lines - 1)  # (-1+ avarage_margin ) # /4   #  (-1+ avarage_margin )/4
...
class Action_Converters_sub:
    def __init__(self, env, sub_id, N_1):
        ...
        action_space = build_sub_action_space(
            env, sub_id=sub_id, action_type="set", N_1=N_1
        )
        self.action_space = [a.as_dict() for a in action_space]
