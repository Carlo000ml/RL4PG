...
class EpsilonGreedyPolicy(Policy):
    ...
    def __init__(
        ...
        self.half_life = half_life / math.log(
            2
        )  # so when the timesteps are equal to the halflife then exp(-t*ln(2)/t)=exp(-ln(2))=1/2
    ...
    def _decay_halflife(self):
        ...
        self.epsilon = self.min_epsilon + (
            self.epsilon_init - self.min_epsilon
        ) * np.exp(-self.time_step / self.half_life)
...
class BoltzmannPolicy(Policy):
    ...
    def __init__(
        self, action_space_dim, temperature=1.0, decay=0.99, min_temperature=0.1
    ):
        super().__init__(action_space_dim)
    ...
    def select_action(self, q_values):
        ...
        """
        probs = F.softmax(
            q_values / self.temperature, dim=0
