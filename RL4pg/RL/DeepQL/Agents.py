...
class DQN_Agent:
    def __init__(
        ...
        Q_value_estimator_kargs["Q_estimator_params"][
            "output_dim"
        ...
        if self.buffer_type == "basic":
            ...
            self.demonstration_buffer = Basic_PrioritizedReplayBuffer(
                **replay_buffer_kargs
            )
    ...
    def exploit(self, graph, current_obs=None):
        ...
        obs = self.gp_manager.process_graph(graph, train=False, target=False).unsqueeze(
            0
        )
        ...
        if self.bad_action_masking:
            bad_indexes = bad_action_masking(self.action_space, current_obs).to(
                self.device
            )
    ...
    def _standard_training_iteration(self):
        # sample a batch
        state, action, reward, next_state, done, _, _ = self.buffer.sample(
            self.gp_manager.batch_size, way="g-a-r-ng", demonstrations=False
        )
        # process in batch using the main and the target gp and estimators
        main_obs = self.gp_manager.process_batch(
            state, train=True, target=False
        )  # [self.agent_id]
        target_obs = self.gp_manager.process_batch(
            next_state, train=False, target=True
        )  # [self.agent_id]   # useless the train=False but you never know

        # compute loss
        if self.rew_shape:
            loss = self.estimator_manager.compute_loss_BSRS(
                main_obs, action, reward, target_obs, done
            )
        else:
            loss = self.estimator_manager.compute_loss(
                main_obs, action, reward, target_obs, done
            )

        if self.log_loss:
            self.writer.add_scalar("Loss", loss.item(), self.global_training_iter)
    ...
    def _standard_demonstration_training_iteration(self):
        ...
        (
            ...
            batch_state_t_n,
            batch_done_t_n,
        ) = self.demonstration_buffer.sample(
            self.gp_manager.batch_size, way="g-a-r-ng-d", demonstrations=True
        )
        # process in batch using the main and the target gp and estimators
        main_obs = self.gp_manager.process_batch(
            batch_state_t, train=True, target=False
        )  # [self.agent_id]
        target_obs_t_1 = self.gp_manager.process_batch(
            batched_states_t_1, train=False, target=True
        )  # [self.agent_id]   # useless the train=False but you never know
        target_obs_t_n = self.gp_manager.process_batch(
            batch_state_t_n, train=False, target=True
        )  # [self.agent_id]   # useless the train=False but you never know

        # compute loss
        loss = self.estimator_manager.compute_demonstration_loss(
            batch_state_t=main_obs,
        ...
        if self.log_loss:
            self.writer.add_scalar(
                "Demonstration Loss",
                loss.item(),
                self.global_demonstration_training_iter,
            )
    ...
    def standard_learn(self):
        if len(self.buffer) < self.start_training_capacity:
            ...
            if self.policy_type == "epsilon-greedy":
                self.writer.add_scalar(
                    "Epsilon", self.policy.epsilon, self.global_training_count
                )

            else:
                self.writer.add_scalar(
                    "Temperature", self.policy.temperature, self.global_training_count
                )
    ...
    def _prioritized_training_iteration(self):
        # sample a batch
        idxs, batch, weights = self.buffer.sample(
            self.gp_manager.batch_size, way="g-a-r-ng-d", demonstrations=False
        )
        ...
        # compute loss
        if self.rew_shape:
            loss, td_error = self.estimator_manager.compute_loss_with_TD_errors_BSRS(
                batch_state=main_obs,
                batch_action=action,
                batch_reward=reward,
                batch_next_state=target_obs,
                batch_done=done,
                IS_weights=weights,
            )
        else:
            loss, td_error = self.estimator_manager.compute_loss_with_TD_error(
                batch_state=main_obs,
                batch_action=action,
                batch_reward=reward,
                batch_next_state=target_obs,
    ...
    def _prioritized_demonstrations_training_iteration(self):
        # sample a batch
        idxs, batch, weights = self.demonstration_buffer.sample(
            self.gp_manager.batch_size, way="g-a-r-ng-d", demonstrations=True
        )
        ...
        # compute loss
        loss, td_error = self.estimator_manager.compute_demonstration_loss_TD_error(
            batch_state_t=main_obs,
            batch_action_t=batch_action_t,
            batch_rewards_t_n=batch_rewards_t_n,
            batch_done_t=batch_done_t,
    ...
    def learn(self):
        ...
        if self.buffer_type == "prioritized":
            self.prioritized_learn()

    def learn_demonstrations(self):
        if self.buffer_type == "basic":
            self.standard_learn_demonstrations()
        if self.buffer_type == "prioritized":
            self.prioritized_learn_demonstrations()
    ...
    def update_parameters(self, loss):
        ...
        torch.nn.utils.clip_grad_value_(
            self.estimator_manager.main_net.parameters(),
            self.estimator_manager.gradient_clipping,
        )
        torch.nn.utils.clip_grad_value_(
            self.gp_manager.main_gp.parameters(), self.gp_manager.gradient_clipping
        )
    ...
    def grid2op_to_torch(self, grid2op_action):
        ...
        """
        return torch.tensor(
            self.action_space.index(grid2op_action),
            dtype=torch.long,
            device=self.device,
        )
    ...
    def __getstate__(self):
        ...
        if "estimator_manager" in state and hasattr(
            state["estimator_manager"], "optimizer"
        ):
            del state["estimator_manager"].optimizer

        if "estimator_manager" in state and hasattr(
            state["estimator_manager"], "writer"
        ):
