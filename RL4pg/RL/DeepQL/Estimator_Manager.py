...
class Estimator_Manager:
    ...
    def compute_loss(
        self, batch_state, batch_action, batch_reward, batch_next_state, batch_done
    ):
    ...
    def compute_loss_BSRS(
        ...
        with torch.no_grad():
            if self.use_double:
                best_main_actions = (
                    self.compute_main_q_values(batch_state, train=False)
                    .argmax(dim=1, keepdim=True)
                    .detach()
                )
                value_st = (
                    self.compute_target_q_values(batch_state)
                    .gather(1, best_main_actions)
                    .view(-1)
                )
            else:
        ...
        with torch.no_grad():
            ...
            target_q_values = (
                batch_reward
                - self.eta * value_st
                + (1 - batch_done) * self.gamma * (1 + self.eta) * value_st1
            )

        return self.loss(q_values, target_q_values)

    def compute_loss_with_TD_error(
        self,
        batch_state,
        batch_action,
        batch_reward,
    ...
    def compute_loss_with_TD_errors_BSRS(
        self,
        batch_state,
        batch_action,
        batch_reward,
    ...
    def compute_demonstration_loss(
        ...
        with torch.no_grad():
            if self.use_double:
                best_main_actions = (
                    self.compute_main_q_values(batched_states_t_1, train=False)
                    .argmax(dim=1, keepdim=True)
                    .detach()
                )
                target_q_values = (
                    self.compute_target_q_values(batched_states_t_1)
                    .gather(1, best_main_actions)
                    .view(-1)
                )
            else:
        ...
        with torch.no_grad():
            if self.use_double:
                best_main_actions = (
                    self.compute_main_q_values(batch_state_t_n, train=False)
                    .argmax(dim=1, keepdim=True)
                    .detach()
                )
                target_q_values = (
                    self.compute_target_q_values(batch_state_t_n)
                    .gather(1, best_main_actions)
                    .view(-1)
                )
            else:
            ...
            rewards = torch.sum(
                batch_rewards_t_n * self.gamma**exponents, axis=1
            )  # shape (batch_size)
        ...
        margin[
            indexes, batch_action_t.view(-1)
        ] = 0  #  place the zeros on the expert actions
        supervised_loss = torch.clip(
            torch.max(self.compute_main_q_values(batch_state_t, train=True) + margin)
            - q_values,
            0,
        ).mean()
    ...
    def compute_demonstration_loss_TD_error(
        self,
        batch_state_t,
        batch_action_t,
        batch_rewards_t_n,
        ...
        n_step_loss = (
            torch.tensor(IS_weights, device=self.device) * TD_n_error.pow(2)
        ).sum()
