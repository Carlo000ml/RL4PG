...
class DQNetwork(nn.Module):
    def __new__(
        ...
        return CustomRegressor(
            input_dim, output_dim, hidden_layers, dropout, use_batch_norm, activation
        )
...
class DuelingQNetwork(nn.Module):
    ...
    def __init__(
        ...
        input_dim,
        output_dim,
        hidden_layers_shared_structure=None,
        structure_value_stream=None,
        structure_advantage_stream=None,
        dropout=0.0,
        use_batch_norm=False,
        activation="leaky_relu",
        advantage_comparison="mean",
    ):
        super(DuelingQNetwork, self).__init__()
        assert (
            structure_value_stream[0] == structure_advantage_stream[0]
        ), "First dimension of value stream and advantage stream must be equal"
        assert advantage_comparison in [
            "mean",
            "max",
        ], "advantage_comparison must be either 'mean' or 'do nothing'"

        self.feature_extractor = CustomRegressor(
            input_dim=input_dim,
            output_dim=structure_value_stream[0],
            hidden_layers=hidden_layers_shared_structure,
            dropout=dropout,
            use_batch_norm=use_batch_norm,
            activation=activation,
        )
        self.value_stream = CustomRegressor(
            input_dim=structure_value_stream[0],
            output_dim=1,
            hidden_layers=structure_value_stream[1:],
            dropout=dropout,
            use_batch_norm=use_batch_norm,
            activation=activation,
        )
        self.advantage_stream = CustomRegressor(
            input_dim=structure_advantage_stream[0],
            output_dim=output_dim,
            hidden_layers=structure_advantage_stream[1:],
            dropout=dropout,
            use_batch_norm=use_batch_norm,
            activation=activation,
        )
