...
def exploit_episode(Agents, MA_Controller, Env, options={}):
    ...
    while not done:
        ...
        if safe_state:
            (next_obs, next_graph), reward, done, info = Env.step(
                MA_Controller.do_nothing()
            )
        else:
            if MA_Controller.check_disconnections(obs):
                ...
                (next_obs, next_graph), reward, done, info = Env.step(action)
                reward = (
                    np.average((np.maximum(1 - next_obs.rho, 0)) ** 2)
                    if not done
                    else -1
                )
    ...
    while not done:
        ...
        if safe_state:
            ...
            if MA_Controller.check_disconnections(obs):
                ...
                Agents[sub_id].store_experience(
                    (
                        graph,
                        Agents[sub_id].grid2op_to_torch(action),
                        line_rew,
                        next_graph,
                        done,
                    )
                ...
                if not exploration:
                    MA_Controller.store_experience(
                        experience=(
                            MA_Controller.obs_to_torch(obs),
                            MA_Controller.playable.index(sub_id),
                            line_rew,
                            MA_Controller.obs_to_torch(next_obs),
                            done,
                        )
                    )
        ...
        cum_rew += (
            np.average((np.maximum(1 - next_obs.rho, 0)) ** 2) if not done else -1
        )
        ep_len += 1
        obs = next_obs
        graph = next_graph
