...
def obs_to_torch(obs, device):
    return torch.tensor(
        np.concatenate([obs.rho, obs.a_or, obs.a_ex, obs.topo_vect], axis=0),
        device=device,
        dtype=torch.float32,
    )
...
def el_to_vect(obs, el, element_type, device="cpu", done=False):
    ...
    if element_type == "load":
        feat = np.hstack([obs.load_p[el], obs.load_q[el], obs.load_v[el]])
    if element_type == "ren gen":
        feat = np.hstack(
            [
                obs.gen_p[el],
                ...
                obs.gen_pmin[el],
                obs.gen_pmax[el],
                obs.gen_p_before_curtail[el],
                obs.curtailment_limit[el],
                obs.curtailment_limit_effective[el],
                obs.curtailment_mw[el],
            ]
        )
    if element_type == "non ren gen":
        feat = np.hstack(
            [
                ...
                obs.gen_pmin[el],
                obs.gen_pmax[el],
                obs.gen_max_ramp_up[el],
                obs.gen_max_ramp_down[el],
                obs.gen_min_uptime[el],
                obs.gen_min_downtime[el],
                obs.gen_cost_per_MW[el],
                obs.gen_startup_cost[el],
                obs.gen_shutdown_cost[el],
                obs.target_dispatch[el],
                obs.actual_dispatch[el],
                obs.gen_margin_up[el],
                obs.gen_margin_down[el],
            ]
        )
    if element_type == "line":
        feat = np.hstack(
    ...
    if element_type == "bus":
        if done:
            ...
            matching_key = next(
                (
                    key
                    for key, value in dict(Net.nodes(data="global_bus_id")).items()
                    if value == el
                ),
                None,
            )
            if matching_key != None:
                feat = np.array(
                    [
                        Net.nodes(data="p")[matching_key],
                        Net.nodes(data="q")[matching_key],
                        Net.nodes(data="v")[matching_key],
                    ]
                )
            else:
                feat = np.zeros(3)
...
def el_2_vec_bus_shift(obs, el, element_type, bus, device="cpu", done=False):
    el_vec = el_to_vect(
        obs=obs, el=el, element_type=element_type, device=device, done=done
    )
...
def substation_vectors(obs, device="cpu", done=False):
    ...
    for id in range(n_sub):  # bus 1 0-14
        substation_embeddings[id, :] += el_2_vec_bus_shift(
            obs, id, "bus", bus=1, device=device, done=done
        )

    for id in range(n_sub):  # bus 2  14-28
        substation_embeddings[id, :] += el_2_vec_bus_shift(
            obs, n_sub + id, "bus", bus=2, device=device, done=done
        )

    for id in range(obs.n_load):
        bus = obs.load_bus[id]
        sub = obs.load_to_subid[id]
        substation_embeddings[sub, :] += el_2_vec_bus_shift(
            obs, id, "load", bus=bus, device=device, done=done
        )
    ...
    for id in range(obs.n_gen):
        ...
        if obs.gen_renewable[id]:
            substation_embeddings[sub, :] += el_2_vec_bus_shift(
                obs, id, "ren gen", bus=bus, device=device, done=done
            )
        else:
            substation_embeddings[sub, :] += el_2_vec_bus_shift(
                obs, id, "non ren gen", bus=bus, device=device, done=done
            )
    ...
    for id in range(obs.n_line):  # origin of line
        ...
        substation_embeddings[sub_or, :] += el_2_vec_bus_shift(
            obs, id, "line", bus=bus_or, device=device, done=done
        )
        substation_embeddings[sub_ex, :] += el_2_vec_bus_shift(
            obs, id, "line", bus=bus_ex, device=device, done=done
        )
...
def sub_connectivity(obs, as_dict=False):
    ...
    sub_edge_index = torch.tensor(
        np.vstack([obs.line_or_to_subid, obs.line_ex_to_subid]), dtype=torch.long
    )

    if (
        as_dict
    ):  # to have (substation extremities) -> line_id  // here we put also the reversed substation extremities mapped to the same line_id
        d = {}
        for i in range(obs.n_line):
            if (sub_edge_index[0][i].item(), sub_edge_index[1][i].item()) not in list(
                d.keys()
            ):
                ...
                if (
                    type(d[(sub_edge_index[0][i].item(), sub_edge_index[1][i].item())])
                    == int
                ):
                    d[(sub_edge_index[0][i].item(), sub_edge_index[1][i].item())] = [
                        d[(sub_edge_index[0][i].item(), sub_edge_index[1][i].item())],
                        i,
                    ]
                else:
                    d[
                        (sub_edge_index[0][i].item(), sub_edge_index[1][i].item())
                    ].append(i)
    ...
    if multigraph:
        ...
        for edg in nx_graph.edges:
            ...
            lineids = np.where(
                np.all(
                    (
                        np.transpose(sub_connectivity(obs).numpy())
                        == np.array([sub0, sub1])
                    )
                    | (
                        np.transpose(sub_connectivity(obs).numpy())
...
def build_sub_action_space(env, sub_id, action_type="set", N_1=False):
    ...
    return env.action_space.get_all_unitary_topologies_set(
        env.action_space, sub_id=sub_id, add_alone_line=False
    ) + [env.action_space({})]
    ...
    if action_type == "set":
        action_space = (
            env.action_space.get_all_unitary_topologies_set(
                env.action_space,
                sub_id=line_extremities[0].item(),
                add_alone_line=False,
            )
            + env.action_space.get_all_unitary_topologies_set(
                env.action_space,
                sub_id=line_extremities[1].item(),
                add_alone_line=False,
            )
            + [env.action_space({})]
        )
...
def fake_action(current_obs, act):
    ...
    return np.all(
        current_obs.topo_vect[affected_elements]
        == act._set_topo_vect[affected_elements]
    )
...
def action_masking(action_space, obs):
    ...
    """
    return torch.tensor(
        np.nonzero(np.array([fake_action(obs, a) for a in action_space]))[0][:-1]
    )
...
def bad_action_masking(action_space, obs):
    ...
    """
    return torch.tensor(
        np.nonzero(np.array([bad_action(obs, a) for a in action_space]))[0]
    )
        ...
        def build_tree(idx, prefix="", is_tail=True):
            ...
            if left_idx >= len(self.tree):
                ...
                return (
                    prefix
                    + ("└── " if is_tail else "├── ")
                    + f"({self.tree[idx]}, {self.data[data_idx]})\n"
                )
            else:
                # For internal nodes, show the sum stored at the node.
                result = (
                    prefix + ("└── " if is_tail else "├── ") + f"{self.tree[idx]}\n"
                )
...
def collect_n_step_expert_experiences(
    episode,
    n,
    action_converters,
    build_graph,
    experiences={i: [] for i in range(20)},
    device="cpu",
):
                ...
                if action.as_dict() in action_converters[line_id].action_space:
                    ...
                    for i in range(2, n):
                        ...
                        agent_next_reward = np.average(
                            (np.maximum(1 - next_obs.rho, 0)) ** 2
                        )  # starting from reward t+1  until reward t+n-1
                        rewards.append(agent_next_reward)
                        if agent_next_reward == 1:
                            done_t_n = 1

                    experiences[line_id].append(
                        (
                            graph,
                            agent_action.item(),
...
def collect_n_step_expert_experiences_RL_Manager(
    episode,
    n,
    action_converters,
    build_graph,
    ...
    for time_step in range(len(episode.actions) - n):
        ...
        if not action.as_dict() == {} and np.all(action.set_line_status == 0):
            ...
            agents_selected = np.concatenate(
                (
                    np.where(obs.line_or_to_subid == sub_id)[0],
                    np.where(obs.line_ex_to_subid == sub_id)[0],
                )
            )
...
def collect_n_step_expert_experiences_RL_Manager_sub(
    episode,
    n,
    action_converters,
    build_graph,
...
def check_at_least_two_lines(action, sub_id):
    bus_connection = np.array(
        action.as_serializable_dict()["set_bus"]["lines_or_id"]
        + action.as_serializable_dict()["set_bus"]["lines_ex_id"]
    )[:, 1]
...
def N_1_secure_action_space(env, sub_id):
    sub_id_action_space = env.action_space.get_all_unitary_topologies_set(
        env.action_space, sub_id=sub_id, add_alone_line=False
    )
    N_1_secure_a_s = []
